...
module.exports = app => {
  ...
  app.get("/images", (req, res) => {
    fs.readdir("./files", function(err, files) {
      ...
      res.status(200).json({
        files: files.filter(
          file => fs.statSync(`./files/${file}`).isFile() && file[0] !== "."
        )
      });
    });
  });
  ...
  app.post("/uv", (req, res) => {
    const { name } = req.body;
    fs.writeFile(`uv/${name}.json`, JSON.stringify(req.body), {
      encoding: "utf-8"
    });
    res.status(200).json({ msg: "アップロード完了" });
  });
  ...
  app.get("/uv", (req, res) => {
    const json = fs.readFileSync(`./uv/${req.query.name}.json`, {
      encoding: "utf-8"
    });
    res.setHeader("Access-Control-Allow-Origin", "*");
  ...
  app.get("/uv-list", (req, res) => {
    fs.readdir("./uv", function(err, files) {
      ...
      res.status(200).json({
        files: files.filter(
          file => fs.statSync(`./uv/${file}`).isFile() && file[0] !== "."
        )
      });
    });
  });
